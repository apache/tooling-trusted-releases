...
_IMPORT_DIR: Final = "catalogue-imports"
...
def _import_root() -> safe.StatePath:
    return paths.get_tmp_dir() / _IMPORT_DIR


def _sweep_stale() -> None:
    root = _import_root().path
    if not root.is_dir():
        return
...
def _token_dir(token: str) -> pathlib.Path:
    if not _TOKEN_RE.fullmatch(token):
        raise KeyError(token)
    return (_import_root() / token).path
