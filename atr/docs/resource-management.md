...
The manager ([`manager`](/ref/atr/manager.py)) tracks four to eight workers, each of which works on 8 to 16 tasks before exiting. Workers claim the oldest known task. The queue is global: there is no user quota, fairness, etc. Workers request, but are not guaranteed, a lifetime limit of 300 processor seconds (which children inherit, with separate accounting), per worker rather than per task. Each worker also runs a watchdog thread, polling every 500ms, which kills the worker's own process group when the resident memory of its process tree exceeds 3 GB.
