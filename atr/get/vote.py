...
async def render_vote_closed_page(release: sql.Release) -> str:
    ...
    redirect_url = util.as_url(selected, project_name=release.project.key, version_name=release.version)
...
def _render_checklist_card(page: htm.Block, release: sql.Release) -> None:
    ...
    checklist_url = util.as_url(checklist.selected, project_name=release.project.key, version_name=release.version)
    body.div[
        htpy.a(
...
def _render_section_checks(page: htm.Block, release: sql.Release, file_totals: checks.FileStats) -> None:
    ...
    body.div[
        htpy.a(
            ".btn.btn-outline-primary",
            href=util.as_url(checks.selected, project_name=release.project.key, version_name=release.version),
...
def _render_vote_unauthenticated(page: htm.Block, release: sql.Release, archive_url: str | None) -> None:
    ...
    if archive_url:
        ...
        committee_name = release.committee.key if release.committee else "unknown"
