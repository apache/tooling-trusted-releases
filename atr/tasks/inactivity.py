...
def _preview_escalation_body(plan: Plan) -> str:
    ...
    return (
        f"As of the end of May 2026, ATR is enacting a new policy where release\n"
        f"previews inactive for more than {delete_days} days are flagged. This release\n"
        f"preview has now been inactive for at least {delete_days} days, and is being\n"
        f"flagged. Release previews are not automatically deleted. The Finish\n"
        f"phase has a form to reset the inactivity clock. Please complete the\n"
        f"finish step or cancel the release.\n\n"
...
def _warning_body(plan: Plan) -> str:
    ...
    remaining_days = delete_days - warn_days
    if plan.phase == sql.ReleasePhase.RELEASE_PREVIEW:
        return (
            f"As of the end of May 2026, ATR is enacting a new policy where release\n"
            f"previews inactive for more than {delete_days} days are flagged. Release\n"
            f"previews are not automatically deleted. This message is being sent at\n"
            f"{warn_days} days (or more) to warn you of this. The Finish phase has a form\n"
            f"to reset the inactivity clock to avoid this release being flagged in\n"
    ...
    return (
        f"As of the end of May 2026, ATR is enacting a new policy where releases\n"
        f"inactive for more than {delete_days} days are deleted. This message is being\n"
        f"sent at {warn_days} days (or more) to warn you of this. The Compose and Finish\n"
        f"phases have a form to reset the inactivity clock if you want to keep\n"
        f"this release. Otherwise it will be deleted in {remaining_days} days.\n\n"
        f"Project: {plan.project_key}\n"
        f"Version: {plan.version}\n"
        f"Phase: {plan.phase.value}\n"
    )
