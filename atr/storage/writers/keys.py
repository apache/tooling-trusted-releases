...
class FoundationCommitter(GeneralPublic):
    ...
    async def _publish_keys_to_svn(self, committee: sql.Committee, source: pathlib.Path) -> outcome.Outcome[None]:
    ...
    async def _sync_committee_keys_file(self, committee_key: str) -> tuple[str | None, outcome.Outcome[None]]:
        committee = await self.__data.committee(key=committee_key, _public_signing_keys=True).demand(
            storage.AccessError(f"Committee not found: {committee_key}", status=404)
        )
...
class CommitteeParticipant(FoundationCommitter):
    ...
    async def associate_fingerprint(self, fingerprint: str) -> outcome.Outcome[datatypes.LinkedCommittee]:
        ...
        try:
            autogenerated_outcome, _ = await self.autogenerate_keys_file()
        except Exception as e:
            return outcome.Error(e)
    ...
    async def autogenerate_keys_file(
        self,
    ) -> tuple[outcome.Outcome[str], outcome.Outcome[None]]:
        no_svn: outcome.Outcome[None] = outcome.Result(None)
    ...
    async def ensure_associated(self, keys_file_text: str) -> outcome.List[datatypes.Key]:
    ...
    async def ensure_stored(self, keys_file_text: str) -> outcome.List[datatypes.Key]:
