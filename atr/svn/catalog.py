...
async def catalogue_commit(commit: dict) -> None:
    ...
    async with db.session() as data:
        releases, supersessions, archives = await _resolve_changes(rules, data, added, removed, changes.removed_files)
...
async def _resolve_changes(
    ...
    for rel_files in sorted(added.values(), key=lambda r: (r.committee, r.subproject or "", r.version)):
        resolution = await _resolve_release(rules, data, rel_files)
...
async def _resolve_release(rules: dist.DistRules, data: db.Session, rel_files: _ReleaseFiles) -> _ReleaseResolution:
