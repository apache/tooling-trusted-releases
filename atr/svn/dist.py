...
rows in the distrule table, loaded into a DistRules snapshot (see atr/svn/dist_rules.py)
...
# provider *areas* (the drop dirs) are a tunable, so they live in the distrule table instead
...
@dataclasses.dataclass(frozen=True)
class DistRules:
    # A snapshot of the decomposer's tunables, loaded once per operation from the distrule
    # table. Treated as read-only so a commit or a backfill pass decomposes against one consistent
