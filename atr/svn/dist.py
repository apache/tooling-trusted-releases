...
# Dirs that hold a project's sub-packages or builds rather than naming a subproject, so the
# subproject and version live in the filename (airflow's providers/, a source/ or binaries/
# build split, a releases/ container). Matched case-insensitively against the first dir
_GROUPING_BUCKETS: Final[frozenset[str]] = frozenset(
    {"providers", "source", "sources", "binaries", "bin", "src", "releases"}
)

# Buckets that only apply under a specific committee, where the name is a real subproject
# elsewhere (maven/plugins holds one release per plugin, but plugins isn't always a bucket)
_COMMITTEE_BUCKETS: Final[frozenset[tuple[str, str]]] = frozenset({("maven", "plugins")})

# Build/status tokens that leak onto a filename's name (chukwa-src, chukwa-incubating-src,
# a stray v from a v-prefixed version); stripped from the subproject only, never the filename
