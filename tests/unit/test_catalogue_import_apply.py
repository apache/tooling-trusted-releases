...
@pytest.mark.asyncio
async def test_store_uploads_saves_each_picker_and_reads_back(import_root: pathlib.Path) -> None:
    files = {
    ...
    assert not (import_root / token).exists()


def test_sweep_removes_stale_upload_dirs_and_keeps_recent_ones(import_root: pathlib.Path) -> None:
    # A previewed-but-never-applied upload is reclaimed once it ages past the TTL
    import_root.mkdir()
    stale = import_root / ("a" * 32)
    recent = import_root / ("b" * 32)
    stale.mkdir()
    recent.mkdir()
...
def test_uploads_reject_a_path_traversal_token(import_root: pathlib.Path) -> None:
